...
from bot.helper.ext_utils.bot_utils import get_content_type, sync_to_async
...
async def telegram_mediainfo(client, message, isRaw):
    ...
    try:
        ...
        with open(f"{download_path}.txt", "r+") as file:
            content = file.read()
        title = "GENERATED BY PIKABOT"
        output = mediainfo_paste(text=content, title=title)
        buttons = ButtonMaker()
...
async def mediainfo(client, message: Message):
    mess = f"<b>Generate media info dari media telegram atau dari direct link.</b> \n\nBalas pesan media dengan perintah <code>/{BotCommands.MediaInfoCommand[0]}</code>"

    rply = message.reply_to_message
    if rply and rply.video or rply and rply.document or rply and rply.document or rply and rply.voice:
        isRaw = False
        ...
        return await sendMessage(message, "Mediainfo dari link sementara diNon-Aktifkan.")

    elif len(message.command) < 2:
        return await sendMessage(message, mess)
